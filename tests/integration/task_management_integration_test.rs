...
impl TestContext {
    ...
    async fn cleanup(&self) {
        ...
        for &task_id in &self.created_task_ids {
            if let Ok(task_id_vo) = TaskId::new(task_id) {
                let _ = self.task_repository.delete_by_id(task_id_vo).await;
            }
        }

        // 作成したプロジェクトを削除
        for &project_id in &self.created_project_ids {
            if let Ok(project_id_vo) = ProjectId::new(project_id) {
                let _ = self.project_repository.delete_by_id(project_id_vo).await;
            }
        }
    }
}
...
async fn test_task_hierarchy_consistency_with_database() {
    ...
    // 各プロジェクトにタスクを作成
    let task_a1 = ctx.create_test_task(project_a.id(), "タスクA1").await;
    let task_a2 = ctx.create_test_task(project_a.id(), "タスクA2").await;
